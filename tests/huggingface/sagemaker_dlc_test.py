...
def run_test(args):
    ...
    elif args.image_type == "TGILLAMACPP":
    ...
    try:
        ...
        if args.image_type == "TGILLAMACPP":
            # First endpoint with default environment (without GGUF)
            logging.info("Deploying first endpoint with default environment (without GGUF)")
            ...
            logging.info("Deploying second endpoint with llamacpp environment (with GGUF)")
    ...
    if image_type == "TGI":
        ...
        if device_type == "gpu":
            return [
                ...
                ("sentence-transformers/all-MiniLM-L6-v2", None, "ml.g5.12xlarge", None)
            ]
        ...
        if device_type == "gpu":
            return [("HuggingFaceTB/SmolLM2-1.7B-Instruct", None, "ml.g5.12xlarge", "HuggingFaceTB/SmolLM2-1.7B-Instruct-GGUF")]
        elif device_type == "cpu":
            return [("HuggingFaceTB/SmolLM2-1.7B-Instruct", None, "ml.g5.12xlarge", "HuggingFaceTB/SmolLM2-1.7B-Instruct-GGUF")]
        else:
            raise ValueError(f"No testing models found for {image_type} on instance {device_type}. "
        ...
        raise ValueError("Invalid image type. Supported types are 'TGI', 'TEI', and 'TGILLAMACPP'.")
