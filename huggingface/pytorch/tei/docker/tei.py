...
def build(configs: ReleaseConfigs):
    ...
    for config in configs.releases:
        ...
        hf_tei_repo = git.Repo.clone_from(GIT_REPO_TEI_URL, GIT_REPO_TEI_LOCAL_FOLDER_NAME, no_checkout=True)
        hf_tei_repo_tag = GIT_REPO_TEI_TAG_PATTERN.format(version=config.version)
        hf_tei_repo.git.checkout(hf_tei_repo_tag)
        LOG.info(f"Checked out {hf_tei_repo} with tag: {hf_tei_repo_tag} to {GIT_REPO_TEI_LOCAL_FOLDER_NAME}.")
        shutil.copytree(GIT_REPO_DOCKERFILES_ROOT_DIRECTORY,
            os.path.join(GIT_REPO_TEI_LOCAL_FOLDER_NAME, GIT_REPO_DOCKERFILES_ROOT_DIRECTORY))
        LOG.info(f"Copied '{GIT_REPO_DOCKERFILES_ROOT_DIRECTORY}' directory to TEI directory for 'COPY' command.")
